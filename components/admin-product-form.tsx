...
const PURITIES = ['585', '750', '925', '950', '999', 'steel']
...
const STONES = [
  ...
  { value: 'cats-eye', label: 'Кошачий глаз' },
  { value: 'citrine', label: 'Цитрин' },
  { value: 'amethyst', label: 'Аметист' },
  { value: 'larimar', label: 'Ларима' },
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select className={field} value={form.collectionId}
            ...
            {(() => {
              const kyzSubs = collections.filter(c => c.id.startsWith('kyz-uzatu-'))
              const others = collections.filter(c => !c.id.startsWith('kyz-uzatu-') && c.id !== 'kyz-uzatu')
              return (
                <>
                  {others.map(c => <option key={c.id} value={c.id}>{c.nameRu}</option>)}
                  {kyzSubs.length > 0 && (
                    <optgroup label="— Кыз узату">
                      {kyzSubs.map(c => <option key={c.id} value={c.id}>{c.nameRu}</option>)}
                    </optgroup>
                  )}
                </>
              )
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select className={field} value={form.purity} onChange={e => set('purity', e.target.value)}>
            ...
            {PURITIES.map(p => <option key={p} value={p}>{p === 'steel' ? 'Мед. сталь' : p}</option>)}
          </select>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-3 mb-3">
          ...
          {uploading > 0 && Array.from({ length: uploading }).map((_, i) => (
            <div key={`uploading-${i}`} className="w-24 h-24 rounded bg-zinc-800 flex items-center justify-center text-zinc-500 text-xs animate-pulse">
              ...
            </div>
          ))}
          <button type="button" onClick={() => fileRef.current?.click()}
            ...
            <span className="text-2xl">+</span><span>Фото</span>
          </button>
        </div>
        ...
        <p className="text-zinc-600 text-xs">Любой формат изображений — до 100MB на файл</p>
      </div>
